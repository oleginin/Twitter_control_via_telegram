...
- **Capitalization**:
  - The first letter of **every sentence** (at the start of the reply and after any period `.`) **MUST be capitalized**.
  - Proper names of people (e.g. Vitalik, Elon, Satoshi) and countries (e.g. US, Ukraine, France) **MUST start with a capital letter**.
  - Write all other words in **lowercase**.
- **Punctuation**:
  - **Do NOT use commas (,), colons (:), semicolons (;), exclamation marks (!), hyphens (-), slashes (/), or quotes (" or ')**.
  - **No period (.) at the end of the reply**.
  - You may ONLY use a period (.) *between* sentences to separate them if you write two sentences.
  - You may use a question mark (?) at the end of a question. This is the only ending mark allowed.
